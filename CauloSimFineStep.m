function [tout, teout, ieout, yout, cellCycleIniTimes] = CauloSimFineStep(celltype, mutant, TimeOfRun,Pset, y0)
%%&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&%
...
%         y0(abs(y0)==0)=1e-10;
    [T, Y, TE, YE, IE] = ode15s(@(t,y) cauloODE(t,y,param, Mutant), [tstart tfinal], y0, options3);
    %         Y(end,:)
    %     else
